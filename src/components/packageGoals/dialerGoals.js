...
import NewPackageGoals from "./CreateGoals.js";
...
const DialerWrapperGoals = {
  ...
  dialerClick: () => {
    const dialers = document.querySelectorAll(".dialer-number-goals");
    const backspace = document.querySelector(".box-backspace-goals");
    const span = document.querySelector(".span-dialed-number");
    const dialersNumber = [
      dialers[9],
      dialers[0],
      dialers[1],
    ...
    for (let i = 0; i < dialersNumber.length; i++) {
      const dialer = dialersNumber[i];
